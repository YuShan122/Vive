        ...
        case SurviveSimpleEventType_PoseUpdateEvent: {
            for (const SurviveSimpleObject* it = survive_simple_get_first_object(actx);
                ...
                if (survive_simple_object_get_type(it) == SurviveSimpleObject_OBJECT) {
                    ...
                    if (strcmp(survive_simple_serial_number(it), LH0_serial_number.c_str())) {
                        // printf("set LH0 tf\n");
                    ...
                    if (strcmp(survive_simple_serial_number(it), LH1_serial_number.c_str())) {
                        // printf("set LH1 tf\n");
...
void deleteParam(ros::NodeHandle nh_)
{
    ...
    nh_.deleteParam(node_name + "/LH0_x");
    nh_.deleteParam(node_name + "/LH0_y");
    nh_.deleteParam(node_name + "/LH0_z");
    nh_.deleteParam(node_name + "/LH0_W");
    nh_.deleteParam(node_name + "/LH0_X");
    nh_.deleteParam(node_name + "/LH0_Y");
    nh_.deleteParam(node_name + "/LH0_Z");
    nh_.deleteParam(node_name + "/LH1_x");
    nh_.deleteParam(node_name + "/LH1_y");
    nh_.deleteParam(node_name + "/LH1_z");
    nh_.deleteParam(node_name + "/LH1_W");
    nh_.deleteParam(node_name + "/LH1_X");
    nh_.deleteParam(node_name + "/LH1_Y");
    nh_.deleteParam(node_name + "/LH1_Z");
