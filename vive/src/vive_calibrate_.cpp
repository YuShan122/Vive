...
public:
    Lighthouse();
    Lighthouse(int od_);
    set_transform_from_world(SurvivePose p_);
    send_transform_from_world(tf::TransformBroadcaster br_);
    lookup_transform_to_map(tf::TransformListener lis_);
    lookup_transform_from_map(tf::TransformListener lis_);
    send_transform_to_map(tf::TransformBroadcaster br_);
private:
    int order;
    std::string frame;
    tf::StampedTransform transform_from_world;
    tf::StampedTransform transform_to_mapGreen;
    tf::StampedTransform transform_to_mapBlue;
    tf::StampedTransform transform_from_mapGreen;
    tf::StampedTransform transform_from_mapBlue;
    VIVEPOSE pose_to_map;
    VIVEPOSE pose_from_map;
};
class Point {
public:
    Point();
    Point(ros::NodeHandle nh_);
    set_transform_from_world(SurvivePose p_);
    send_transform_from_world(tf::TransformBroadcaster br_);
    send_transform_to_maps(tf::TransformBroadcaster br_);
    lookup_transform_from_map(tf::TransformListener lis_);
private:
    double x, y;
    std::string frame;
    ...
    tf::StampedTransform transform_to_center;
};
class Map {
public:
    lookup_transform_to_world(tf::TransformListener lis_);
    set_transform_to_world();
    send_transform_to_world(tf::TransformBroadcaster br_);
private:
    std::string frame;
    tf::StampedTransform transform_to_world;
}
