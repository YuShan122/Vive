...
int main(int argc, char** argv) {
    ...
    get_param_ok = nh.param<int>(node_name + "/unit", unit, 1000);

    if (get_param_ok) {
        ROS_INFO("set param ok");
    }
    else
        ROS_INFO("set param failed");

    SurviveSimpleContext* actx = survive_simple_init_with_logger(argc, argv, log_fn);
    if (actx == 0)
        return 0;

    // printf("survive_simple_start_thread\n");
    survive_simple_start_thread(actx);

    for (const SurviveSimpleObject* it = survive_simple_get_first_object(actx); it != 0; it = survive_simple_get_next_object(actx, it)) {
    ...
    while (keepRunning && survive_simple_wait_for_event(actx, &event) != SurviveSimpleEventType_Shutdown && ros::ok()) {
        ...
        case SurviveSimpleEventType_PoseUpdateEvent: {
            ...
            printf("%s %s (%7.3f): %f %f %f %f %f %f %f\n", survive_simple_object_name(pose_event->object),
                survive_simple_serial_number(pose_event->object), timecode, pose.Pos[0] * unit, pose.Pos[1] * unit, pose.Pos[2] * unit,
