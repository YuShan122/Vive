...
void Vive::timerCallback(const ros::TimerEvent& e)
{
    ...
    else
    {
        ...
        survivie_tf = lookup_tf(survive_lh1_frame_, survive_world_frame_);
        publish_tf(lh_origin_frame_prefix_ + std::to_string(1), lh_frame_prefix_ + std::to_string(1), survivie_tf);
        survivie_tf = lookup_tf(survive_lh2_frame_, survive_world_frame_);
...
{
    ros::init(argc, argv, "lighthouse_tf_plus");
    ros::NodeHandle nh("");
